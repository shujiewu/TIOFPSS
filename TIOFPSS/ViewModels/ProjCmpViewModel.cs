...
{
    public class ProjCmpViewModel:ViewModel
    {
        public List<UserProject> projects;

        public ProjCmpViewModel()
        {
            projects = new List<UserProject>();
        }
        public List<string> para = new List<string>();
        public ProjCmpViewModel(List<string> path)
        {
            projects = new List<UserProject>();
            
            for(int i=0;i<path.Count;i++)
            {
                UserProject userProject = new DB.UserProject();
                string paraPath = System.IO.Path.Combine(path[i], "project\\参数文件\\parameter.xml");
                para.Add("0");
                para.Add(path[i]);
                para.Add(path[i]);
        ...
        public void loopSetValue(List<string> value,UserProject userProject)
        {
            //摩擦片设计参数 10                           
